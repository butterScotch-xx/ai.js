 ...
 * @param {Array} arr The array to dimensionalize
 * @return {Array} out And array of arrays with content from provided array
 */

function dimensionalize(dm, arr) {
    if (__(dm).reduce(function(a, b) {
        return a * b;
    }) !== arr.length) throw new Error('Dimensions do not match');

    if (dm.length === 1) return arr;
        ...
        for (var i = 0; i < dm[0]; i++) {
            ...
            for (var j = 0; j < dm[1]; j++) {
                ...
                for (var k = 0; k < dm[2]; k++) {
                    a[i][j].push(arr[(dm[1] * i + dm[2] * j + k)]);
                }
            }
        }
        return a;
